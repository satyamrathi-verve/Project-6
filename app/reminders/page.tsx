...
const touchpointClass: Record<string, string> = {
  ...
  note: "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400",
};
...
const agingClass: Record<AgingBucket, string> = {
  "Not due": "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400",
  "0-30 days": "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400",
  "31-60 days": "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400",
    ...
    { key: "invoice_no", header: "Invoice #", sortable: true },
    {
      ...
      render: (row) => (
        <div>
          ...
          {row.customers?.email ? (
            <div className="text-xs text-slate-400 dark:text-slate-500">{row.customers.email}</div>
          ) : (
    ...
    },
    {
      ...
      render: (row) => {
        ...
        const cfg: Record<FollowUpStatus, { label: string; cls: string }> = {
          ...
          on_track: { label: "Recently reminded", cls: "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400" },
        };
        return (
              ...
              <>
                ...
                <span
                  className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                    latestIsRecentSend ? "bg-amber-100 text-amber-700" : touchpointClass[latest.status] ?? "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400"
                  }`}
                >
    ...
    <>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            <section className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
              <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Overdue invoices</h3>
            ...
            <aside className="space-y-6">
              <section className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Reminder template</h3>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Choose which template to send. Edit wording on the Reminder Template screen.</p>
                <div className="mt-4">
                  ...
                  {selectedTemplate && (
                    <p className="mt-3 text-sm text-slate-600 dark:text-slate-400 line-clamp-2">{selectedTemplate.subject}</p>
                  )}
                </div>
              </section>
              ...
              <section className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
                <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{historyTitle}</h3>
                <div className="mt-4 space-y-3 text-sm text-slate-700 dark:text-slate-300">
                  {historyEntries.length === 0 ? (
                    <p className="text-slate-500 dark:text-slate-400">No touchpoints have been logged yet.</p>
                  ) : (
                    historyEntries.map((entry) => (
                      <div key={entry.id} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/40 p-3">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span
                            className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold ${
                              touchpointClass[entry.status] ?? "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400"
                            }`}
                          >
                            {touchpointLabel[entry.status] ?? entry.status}
                          </span>
                        </div>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{formatDateTime(entry.sent_at)}</p>
      ...
      {confirmDuplicates && (
        <Modal title="Already reminded recently" onClose={() => setConfirmDuplicates(false)}>
          ...
          <ul className="mt-3 max-h-40 space-y-1 overflow-y-auto text-sm text-slate-600 dark:text-slate-400">
